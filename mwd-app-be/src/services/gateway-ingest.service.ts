...
const recentGatewayPackets = new Map<string, number>();
...
const getDepthBasis = (
  parsedFields: ReturnType<typeof parseMeasurementFields> extends infer Result
    ? Result extends { parsedFields: infer Fields }
      ? Fields
      : never
    : never,
) => {
  const fields = parsedFields as Record<string, { value?: unknown }>;
  return fields.depthMd?.value ?? fields.hole_depth?.value ?? null;
};

const isNewDepthSnapshot = (
  latestDepth: unknown,
...
const normalizeMeasurementSignature = (
  parsedFields: ReturnType<typeof parseMeasurementFields> extends infer Result
    ? Result extends { parsedFields: infer Fields }
      ? Fields
      : never
  ...
  return Object.entries(parsedFields as Record<string, { value?: unknown }>)
    .map(([key, field]) => [key, field?.value] as const)
...
const cleanupDedupCache = (now: number) => {
...
const getGatewayDedupInfo = (
  ...
  const gatewayTransmitter = parseOptionalText(
...
const isDuplicateGatewayPayload = (
  sessionId: number,
  payload: GatewayPayload,
  measurementSignature: string,
) => {
...
export const ingestGatewayPayloads = async (rawPayload: unknown) => {
  ...
  const items = await prisma.$transaction(async (tx) => {
    ...
    for (const [index, payload] of payloads.entries()) {
      ...
      const measurementSignature = normalizeMeasurementSignature(
        measurementResult.parsedFields,
      );

      if (isDuplicateGatewayPayload(sessionId, payload, measurementSignature)) {
        console.log(
          `[Gateway Ingest] Duplicate payload skipped for session ${sessionId}.`,
        );
        continue;
      }
