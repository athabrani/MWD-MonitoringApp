...
const ensureSessionAccess = async (req: Request, res: Response, sessionId: number) => {
...
const buildSurveyConfigInput = (sessionId: number, body: Record<string, unknown>) => {
  const input: surveyConfigService.SurveyConfigInput = { sessionId };
  ...
  if (body.plotTemplateId !== undefined) {
    const plotTemplateId =
      body.plotTemplateId === null || body.plotTemplateId === ""
        ? null
...
export const getSurveyConfig = async (req: Request, res: Response) => {
  try {
...
export const upsertSurveyConfig = async (req: Request, res: Response) => {
  try {
    const user = getAuthUser(req);
    const sessionId = parsePositiveInt(req.params.sessionId);
...
export const deleteSurveyConfig = async (req: Request, res: Response) => {
  try {
    const user = getAuthUser(req);
    const sessionId = parsePositiveInt(req.params.sessionId);
