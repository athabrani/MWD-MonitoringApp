...
import { PDF_STYLE } from './export/templates/pdfStyle.js';
...
const normalizeHeaderFields = (value, fallback) => {
    ...
    const fields = value
        ...
        const source = item.source === 'sessionCode' ||
...
const normalizeTracks = (value, fallback) => {
    if (!Array.isArray(value)) {
        return fallback;
    }
    const tracks = value
...
const toCurveFromTrack = (track, fallbackKey, fallbackLabel, fallbackColor) => {
...
const findTrackByCurveKey = (tracks, key) => {
    return tracks.find((track) => track.curves.some((curve) => curve.key === key));
};
const groupMwdTracks = (tracks) => {
    if (tracks.length <= 4) {
        return tracks;
    }
    ...
    if (pressureTracks.filter(Boolean).length < 2 ||
...
const normalizePageSettings = (value, fallback) => {
    ...
    return {
        ...
        headerHeightFirstPage: toPositiveNumber(page.headerHeightFirstPage, fallback.headerHeightFirstPage),
        headerHeightOtherPages: toPositiveNumber(page.headerHeightOtherPages, fallback.headerHeightOtherPages),
        trackHeaderHeight: toPositiveNumber(page.trackHeaderHeight, fallback.trackHeaderHeight),
    };
};
const normalizeLogo = (value, fallback) => {
    if (value === null) {
        return undefined;
    }
    if (!isRecord(value)) {
...
const mergeTemplateConfig = (fallback, override) => {
    if (!isRecord(override)) {
        return fallback;
    }
    const logo = normalizeLogo(override.logo, fallback.logo);
    const config = {
        title: typeof override.title === 'string' && override.title.trim()
            ? override.title.trim()
            : fallback.title,
        ...
        minorDepthStep: toPositiveNumber(override.minorDepthStep, fallback.minorDepthStep),
...
const colorToRgb = (value, fallback) => {
...
const chooseDepthStep = (preferredStep, depthSpan, maxLines) => {
    if (depthSpan <= 0 ||
        preferredStep <= 0 ||
...
const estimateTextWidth = (value, size) => {
...
const parseJpegImage = (data, name) => {
...
const unfilterPngScanlines = (inflated, width, height, bytesPerPixel) => {
...
const parseImageResource = (dataUrl, name) => {
...
class PdfPageBuilder {
    ...
    line(x1, topY1, x2, topY2, color = '#000000', width = 0.5) {
        this.parts.push(`q ${colorToRgb(color, '#000000')} RG ${width} w ${x1.toFixed(2)} ${this.y(topY1).toFixed(2)} m ${x2.toFixed(2)} ${this.y(topY2).toFixed(2)} l S Q`);
    }
    rect(x, topY, width, height, color = '#000000', lineWidth = 0.5) {
        this.parts.push(`q ${colorToRgb(color, '#000000')} RG ${lineWidth} w ${x.toFixed(2)} ${(this.y(topY) - height).toFixed(2)} ${width.toFixed(2)} ${height.toFixed(2)} re S Q`);
    }
    fillRect(x, topY, width, height, color = '#f2f2f2') {
        this.parts.push(`q ${colorToRgb(color, '#f2f2f2')} rg ${x.toFixed(2)} ${(this.y(topY) - height).toFixed(2)} ${width.toFixed(2)} ${height.toFixed(2)} re f Q`);
    }
    text(value, x, topY, size = 12, color = '#000000', weight = 'regular') {
        const fontResource = weight === 'bold' ? 'F2' : 'F1';
        this.parts.push(`q ${colorToRgb(color, '#000000')} rg BT /${fontResource} ${size} Tf ${x.toFixed(2)} ${(this.y(topY) - size).toFixed(2)} Td (${escapePdfText(value)}) Tj ET Q`);
    }
    textCenter(value, centerX, topY, size = 12, color = '#000000', weight = 'regular') {
        const text = String(value ?? '');
        const x = centerX - estimateTextWidth(text, size) / 2;
        this.text(text, x, topY, size, color, weight);
    }
    image(name, x, topY, width, height) {
        this.parts.push(`q ${width.toFixed(2)} 0 0 ${height.toFixed(2)} ${x.toFixed(2)} ${(this.y(topY) - height).toFixed(2)} cm /${name} Do Q`);
    }
    polyline(points, color = '#000000', lineWidth = 0.8) {
        ...
        this.parts.push(`q ${colorToRgb(color, '#000000')} RG ${lineWidth} w ${path} S Q`);
    }
    dashedPolyline(points, color = '#000000', lineWidth = 0.8, dash = 4, gap = 5) {
        const firstPoint = points[0];
        if (!firstPoint || points.length < 2) {
            return;
        }
        ...
        this.parts.push(`q ${colorToRgb(color, '#000000')} RG ${lineWidth} w [${dash} ${gap}] 0 d ${path} S Q`);
    }
    build() {
...
class SimplePdfDocument {
    ...
    build() {
        ...
        objects.push(Buffer.from(`<< /Type /Font /Subtype /Type1 /BaseFont /${PDF_STYLE.font.regular} >>`, 'utf8'));
        objects.push(Buffer.from(`<< /Type /Font /Subtype /Type1 /BaseFont /${PDF_STYLE.font.bold} >>`, 'utf8'));
        for (const image of this.images) {
            const imageDictionary = `<< /Type /XObject /Subtype /Image /Width ${image.width} /Height ${image.height} ` +
        ...
        for (const [index, page] of this.pages.entries()) {
            ...
            objects.push(Buffer.from(`<< /Type /Page /Parent 2 0 R /MediaBox [0 0 ${page.width.toFixed(2)} ${page.height.toFixed(2)}] /Resources << /Font << /F1 3 0 R /F2 4 0 R >> ${xObjectResource} >> /Contents ${contentObjectId} 0 R >>`, 'utf8'));
            objects.push(Buffer.from(`<< /Length ${Buffer.byteLength(page.content, 'utf8')} >>\nstream\n${page.content}\nendstream`, 'utf8'));
        }
...
const resolveHeaderValue = (field, input) => {
...
const withSessionHeaderValues = (config, metadata) => {
...
const mmToPoints = (value) => value * MM_TO_PT;
const getReportPageSize = () => ({
    width: mmToPoints(A4_WIDTH_MM),
    height: mmToPoints(A4_HEIGHT_MM),
});
const getLongLogPlotPageSize = (config, startDepth, endDepth) => {
    ...
    const fixedChromeHeightPt = config.page.marginTop +
        config.page.headerHeightFirstPage +
        config.page.trackHeaderHeight +
        config.page.marginBottom +
        18;
    const height = Math.max(mmToPoints(MIN_LONG_LOG_HEIGHT_MM), fixedChromeHeightPt + mmToPoints(plottedDepthMm));
    return { width, height };
};
const toPolarisLogPageSettings = (settings) => ({
    ...settings,
...
const BJG_CURVE_HEADER_CONFIGS = {
    annularPressure: {
        key: 'annularPressure',
        label: 'Pressure - Anular',
        unit: '',
...
const applyPolarisCurveStyles = (config) => ({
    ...config,
...
const collectCurveKeys = (tracks) => {
    const fieldNames = new Set(MWD_MEASUREMENT_FIELDS);
    return Array.from(new Set(tracks.flatMap((track) => track.curves
        .map((curve) => curve.key)
        .filter((key) => fieldNames.has(key)))));
};
const isWitsIdKey = (value) => {
    return /^\d{4}$/.test(value.trim());
};
const resolveWitsCurveKeys = async (config) => {
    const fieldNames = new Set(MWD_MEASUREMENT_FIELDS);
    const witsIds = Array.from(new Set(config.tracks.flatMap((track) => track.curves.map((curve) => curve.key.trim()).filter(isWitsIdKey))));
    if (witsIds.length === 0) {
        return config;
    }
    ...
    const configByWitsId = new Map(configs.map((item) => [String(item.witsId), item]));
    return {
        ...config,
        tracks: config.tracks.map((track) => ({
            ...track,
            curves: track.curves.map((curve) => {
                ...
                const mappedField = typeof witsConfig?.mappedField === 'string'
...
const fetchPlotRows = async (input, curveKeys) => {
    const select = {
        measuredAt: true,
        depthMd: true,
    };
...
const resolvePlotDepthRange = (input, config, rows) => {
    ...
    const lastDepth = input.depthMax ?? defaultEndDepth ?? dataEnd ?? firstDepth + config.depthPerPage;
...
const fetchSurveyConfigMetadata = async (sessionId) => {
    try {
...
const fetchSessionMetadata = async (input) => {
    ...
    const readConfigText = (key, fallback) => {
        const value = surveyConfig?.[key];
        return typeof value === 'string' && value.trim()
            ? value.trim()
            : (fallback ?? null);
    };
    const readConfigNumber = (key, fallback = null) => {
        const parsed = toFiniteNumber(surveyConfig?.[key]);
        return parsed !== null ? parsed : fallback;
    };
    return {
        sessionCode: readText('sessionCode', input.sessionCode) ?? input.sessionCode,
        company: readConfigText('companyName', readText('company')),
        wellName: readConfigText('wellName', readText('wellName', input.wellName ?? null)),
        wellId: readText('wellId'),
        rigName: readConfigText('rigName', readText('rigName', input.rigName ?? null)),
        ...
        longitude: readConfigNumber('longitude', toFiniteNumber(session?.longitude)),
...
const fetchSurveyTableRows = async (input) => {
    const depthFilter = {};
    if (input.depthMin !== undefined) {
        depthFilter.gte = input.depthMin;
    }
...
const findNearestRow = (rows, depth) => {
    let nearest = null;
...
const drawHeader = (page, input, metadata, config, width, logo) => {
    ...
    page.rect(marginLeft, headerBoxTop, width - marginLeft - marginRight, headerBoxHeight, PDF_STYLE.color.black, PDF_STYLE.line.trackSeparator);
    // Header dibuat fixed supaya tidak berubah-ubah seperti auto-generated report.
    if (logo) {
        page.image(logo.resource.name, marginLeft + 10, headerBoxTop + 8, logo.width, logo.height);
    }
    page.text(title, Math.max(marginLeft + 72, titleX), 35, titleSize, PDF_STYLE.color.black, 'bold');
    const scaleX = width - marginRight - 42;
    page.text('MD', scaleX + 16, 24, 12, PDF_STYLE.color.black, 'bold');
    page.text(`1:${formatNumber(config.scaleRatio)}`, scaleX + 8, 40, 8, PDF_STYLE.color.black, 'bold');
    const drawMetadata = (items, x, labelWidth) => {
        const yStart = 62;
        const lineHeight = 8.9;
        for (const [index, item] of items.entries()) {
            const y = yStart + index * lineHeight;
            page.text(`${item[0]}:`, x, y, 7, PDF_STYLE.color.black, 'bold');
            page.text(truncate(item[1], 34), x + labelWidth, y, 7, PDF_STYLE.color.black);
        }
    };
    drawMetadata([
        ['Company', formatSurveyText(metadata.company)],
...
const drawTrackHeaders = (page, tracks, getTrackX, getTrackWidth, topY, headerHeight) => {
    const drawCurveSample = (headerConfig, x1, y, x2) => {
    ...
    for (const [index, track] of tracks.entries()) {
        ...
        page.rect(trackX, topY, trackWidth, headerHeight, PDF_STYLE.color.black, PDF_STYLE.line.trackSeparator);
        for (const [curveIndex, curve] of track.curves.entries()) {
            ...
            const labelX = trackX + Math.max(2, (trackWidth - estimateTextWidth(label, 8.4)) / 2);
            ...
            if (unit) {
                ...
                page.text(unitText, unitX, curveTop + 10.4, 5, PDF_STYLE.color.darkGray);
            }
            const sampleY = curveTop + rowHeight - 17;
            drawCurveSample(headerConfig, trackX + trackWidth * 0.27, sampleY, trackX + trackWidth * 0.73);
            ...
            for (const [rowIndex, row] of headerConfig.scaleRows.entries()) {
                ...
                if (row.center) {
                    page.textCenter(row.center, trackX + trackWidth / 2, scaleY, 5, PDF_STYLE.color.black);
                }
                page.text(row.right, trackX + trackWidth - estimateTextWidth(row.right, 5) - 4, scaleY, 5, PDF_STYLE.color.black);
            }
        }
    }
};
const drawMdColumn = (builder, options) => {
    const centerX = options.x + options.width / 2;
    builder.rect(options.x, options.headerTop, options.width, options.headerHeight, PDF_STYLE.color.black, PDF_STYLE.line.trackSeparator);
    builder.textCenter('MD', centerX, options.headerTop + 12, 12, PDF_STYLE.color.black, 'bold');
    builder.line(options.x, options.plotTop, options.x, options.plotBottom, PDF_STYLE.color.black, PDF_STYLE.line.trackSeparator);
    builder.line(options.x + options.width, options.plotTop, options.x + options.width, options.plotBottom, PDF_STYLE.color.black, PDF_STYLE.line.trackSeparator);
    const firstMajor = Math.ceil(options.minDepth / options.majorStep) * options.majorStep;
    for (let depth = firstMajor; depth <= options.maxDepth + 0.0001; depth += options.majorStep) {
        const topY = options.depthToTopY(depth);
        const labelTop = Math.min(options.plotBottom - 18, Math.max(options.plotTop + 2, topY - 7));
        const depthText = formatNumber(depth);
        builder.textCenter(depthText, centerX, labelTop, 7.2, PDF_STYLE.color.black, 'bold');
        ...
        const timestampTop = Math.min(options.plotBottom - 10, Math.max(options.plotTop + 11, labelTop + 12));
        builder.textCenter(datePart ?? '', centerX, timestampTop, 5.1, PDF_STYLE.color.darkGray);
        builder.textCenter(timePart ?? '', centerX, timestampTop + 6, 5.1, PDF_STYLE.color.darkGray);
    }
};
const drawPlotPage = (input, metadata, config, logo, pageIndex, pageCount, pageStartDepth, pageEndDepth, rows, width, height) => {
    const builder = new PdfPageBuilder(width, height);
    const settings = config.page;
    const headerHeight = pageIndex === 0
    ...
    const footerTrackHeaderTop = height - settings.marginBottom - footerTrackHeaderHeight;
    ...
    const mdColumnWidth = config.tracks.length >= 4 ? Math.min(42, trackAreaWidth * 0.072) : 0;
    const baseTrackWeights = [22, 25, 25, 25];
    const trackWeights = config.tracks.map((_, index) => baseTrackWeights[index] ?? 23);
    const totalTrackWeight = trackWeights.reduce((total, value) => total + value, 0);
    const trackWidths = trackWeights.map((weight) => ((trackAreaWidth - mdColumnWidth) * weight) / totalTrackWeight);
    const getTrackWidth = (index) => trackWidths[index] ??
    ...
    const depthToTopY = (depth) => plotTop + ((depth - pageStartDepth) / depthSpan) * plotHeight;
    builder.rect(settings.marginLeft, plotTop, plotRight - settings.marginLeft, plotHeight, PDF_STYLE.color.black, PDF_STYLE.line.normal);
    if (pageIndex === 0) {
    ...
    if (pageIndex === 0) {
        drawTrackHeaders(builder, config.tracks, getTrackX, getTrackWidth, trackHeaderTop, settings.trackHeaderHeight);
    }
    const firstMinorDepth = Math.ceil(pageStartDepth / minorDepthStep) * minorDepthStep;
    for (let depth = firstMinorDepth; depth <= pageEndDepth + 0.0001; depth += minorDepthStep) {
        const topY = depthToTopY(depth);
        const isMajor = Math.abs(depth / depthStep - Math.round(depth / depthStep)) < 0.0001;
        builder.line(settings.marginLeft, topY, plotRight, topY, isMajor ? PDF_STYLE.color.lightGray : PDF_STYLE.color.veryLightGray, isMajor ? 0.65 : 0.22);
    ...
    for (let index = 0; index < config.tracks.length; index += 1) {
        ...
        const trackWidth = getTrackWidth(index);
        builder.line(x, plotTop, x, plotBottom, PDF_STYLE.color.black, PDF_STYLE.line.trackSeparator);
        for (let gridIndex = 1; gridIndex < 10; gridIndex += 1) {
            const gridX = x + (trackWidth / 10) * gridIndex;
            const isMajorGrid = gridIndex === 5;
            builder.line(gridX, plotTop, gridX, plotBottom, isMajorGrid ? PDF_STYLE.color.gray : PDF_STYLE.color.lightGray, isMajorGrid ? 0.55 : 0.25);
        }
    }
    builder.line(plotRight, plotTop, plotRight, plotBottom, PDF_STYLE.color.black, PDF_STYLE.line.trackSeparator);
    if (mdColumnWidth > 0) {
    ...
    if (footerTrackHeaderHeight > 0) {
        drawTrackHeaders(builder, config.tracks, getTrackX, getTrackWidth, footerTrackHeaderTop, footerTrackHeaderHeight);
        if (mdColumnWidth > 0) {
            builder.rect(mdColumnX, footerTrackHeaderTop, mdColumnWidth, footerTrackHeaderHeight, PDF_STYLE.color.black, PDF_STYLE.line.trackSeparator);
            builder.textCenter('MD', mdColumnX + mdColumnWidth / 2, footerTrackHeaderTop + 12, 12, PDF_STYLE.color.black, 'bold');
        }
    }
    builder.text(`MD ${formatNumber(pageStartDepth)} - ${formatNumber(pageEndDepth)} | Page ${pageIndex + 1}/${pageCount} | Scale 1:${config.scaleRatio}`, settings.marginLeft, footerTop, 7, PDF_STYLE.color.darkGray);
    return builder.build();
};
const formatSurveyValue = (value, decimals = 2) => {
    if (value === null) {
        return '-';
    }
...
const formatSurveyText = (value) => {
    if (value === null || value === undefined || value === '') {
        return '-';
    }
    return String(value);
};
const formatLatLong = (metadata) => {
    const latitude = metadata.latitude !== null ? formatSurveyValue(metadata.latitude, 7) : '-';
    const longitude = metadata.longitude !== null ? formatSurveyValue(metadata.longitude, 7) : '-';
    return `${latitude} / ${longitude}`;
};
const drawSurveyCell = (builder, value, x, topY, width, height, options = {}) => {
    ...
    builder.rect(x, topY, width, height, options.bold ? PDF_STYLE.color.gray : PDF_STYLE.color.lightGray, options.bold ? PDF_STYLE.line.thin : 0.18);
    builder.text(text, textX, topY + Math.max(2.2, (height - fontSize) / 2 + 0.6), fontSize, PDF_STYLE.color.black, options.bold ? 'bold' : 'regular');
};
const drawSurveyInfoCell = (builder, label, value, x, topY, width) => {
    const labelWidth = Math.min(62, width * 0.42);
    builder.text(`${truncate(label, 18)}:`, x, topY, 6.3, PDF_STYLE.color.black, 'bold');
    builder.text(truncate(value, Math.max(8, Math.floor((width - labelWidth - 4) / 2.7))), x + labelWidth, topY, 6.3, PDF_STYLE.color.black);
};
const drawDirectionalSurveyHeader = (builder, input, metadata, logo, pageIndex, pageCount, width) => {
    const marginLeft = PDF_STYLE.page.marginLeft;
    const marginRight = PDF_STYLE.page.marginRight;
    ...
    if (logo) {
        const logoWidth = 70;
        const logoHeight = Math.min(30, logoWidth / (logo.resource.width / logo.resource.height));
        builder.image(logo.resource.name, marginLeft, PDF_STYLE.page.marginTop, logoWidth, logoHeight);
    }
    builder.text(wellTitle, Math.max(marginLeft + 80, titleX), 17, 16, PDF_STYLE.color.black, 'bold');
    builder.text(title, Math.max(marginLeft + 80, subtitleX), 35, 10, PDF_STYLE.color.black);
    builder.text('Start date:', width - 116, 17, 7, PDF_STYLE.color.black, 'bold');
    builder.text(formatSurveyDate(metadata.startedAt), width - 68, 17, 7, PDF_STYLE.color.black);
    builder.text('End date:', width - 116, 27, 7, PDF_STYLE.color.black, 'bold');
    builder.text(formatSurveyDate(metadata.endedAt), width - 68, 27, 7, PDF_STYLE.color.black);
    if (pageCount > 1) {
        builder.text(`Page ${pageIndex + 1}/${pageCount}`, width - 64, 40, 6, PDF_STYLE.color.darkGray);
    }
    builder.line(marginLeft, 50, width - marginRight, 50, PDF_STYLE.color.black, PDF_STYLE.line.normal);
    ...
    for (const [columnIndex, column] of columns.entries()) {
        ...
        for (const [rowIndex, field] of column.entries()) {
            drawSurveyInfoCell(builder, field[0], field[1], x, topY + rowIndex * rowGap, columnWidth);
        }
    }
};
const drawSurveyTablePage = (input, metadata, logo, rows, pageRows, pageStartRowIndex, pageIndex, pageCount, width, height) => {
    const builder = new PdfPageBuilder(width, height);
    const marginLeft = PDF_STYLE.page.marginLeft;
    const marginRight = PDF_STYLE.page.marginRight;
    const tableTop = 134;
    ...
    drawDirectionalSurveyHeader(builder, input, metadata, logo, pageIndex, pageCount, width);
    let x = marginLeft;
    ...
    for (const [rowIndex, row] of pageRows.entries()) {
        ...
        const surveyNumber = row.isTieIn && absoluteRowIndex === 0
            ? 'TieIn'
    ...
    if (rows.length === 0) {
        builder.text('No actual survey station data available for this depth range.', marginLeft, tableTop + rowHeight + 12, 8);
    }
    builder.text('Directional Survey Report', marginLeft, height - 30, 6.5, PDF_STYLE.color.darkGray);
    return builder.build();
};
const sanitizeFileName = (value) => {
    return value.replace(/[^a-zA-Z0-9._-]+/g, '_').replace(/^_+|_+$/g, '');
};
export const buildPdfPlot = async (input) => {
    ...
    document.addPage(drawPlotPage(input, sessionMetadata, config, logo, 0, plotPageCount, startDepth, endDepth, rows, plotPageSize.width, plotPageSize.height), plotPageSize.width, plotPageSize.height);
    const surveyRowsPerPage = Math.max(1, Math.floor((reportPageSize.height - 148) / 10.2));
    const surveyPageCount = surveyRows.length > 0 ? Math.ceil(surveyRows.length / surveyRowsPerPage) : 1;
    for (let pageIndex = 0; pageIndex < surveyPageCount; pageIndex += 1) {
        const startRow = pageIndex * surveyRowsPerPage;
        const pageRows = surveyRows.slice(startRow, (pageIndex + 1) * surveyRowsPerPage);
        document.addPage(drawSurveyTablePage(input, sessionMetadata, logo, surveyRows, pageRows, startRow, pageIndex, surveyPageCount, reportPageSize.width, reportPageSize.height), reportPageSize.width, reportPageSize.height);
    }
...
//# sourceMappingURL=pdf-plot.service.js.map