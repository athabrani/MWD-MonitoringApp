...
export const getWitsOutputMessages = async (req, res) => {
    try {
        const authUser = getAuthUser(req);
        ...
        const witsId = req.query.witsId === undefined
            ? undefined
...
export const generateWitsOutputFromLatest = async (req, res) => {
    try {
...
export const markWitsOutputStatus = async (req, res) => {
    try {
        if (!ensureCanModify(req, res)) {
            return;
        }
        ...
        const message = await witsOutputService.updateWitsOutputStatus(id, status, reason);
        res.json(message);
    }
    catch (error) {
        const message = error instanceof Error ? error.message : "Internal server error";
...
//# sourceMappingURL=wits-output.controller.js.map