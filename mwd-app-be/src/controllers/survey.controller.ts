...
const ensureCanWriteSurvey = (req: Request, res: Response) => {
...
const handleSurveyWriteError = (error: unknown, res: Response) => {
  if (
...
export const createSurveyStation = async (req: Request, res: Response) => {
  try {
...
export const getSurveyStations = async (req: Request, res: Response) => {
  try {
    const authUser = getAuthUser(req);
    ...
    const stationType =
      typeof req.query.stationType === "string"
        ? normalizeStationType(req.query.stationType)
        : undefined;
    const filters: surveyService.SurveyStationFilters = {
...
export const getSurveyStationById = async (req: Request, res: Response) => {
  try {
    ...
    const station = (await surveyService.getSurveyStationById(id)) as {
...
export const updateSurveyStation = async (req: Request, res: Response) => {
  try {
    if (!ensureCanWriteSurvey(req, res)) {
      return;
    }
    ...
    const existing = (await surveyService.getSurveyStationById(id)) as {
      sessionId: number;
      session?: { userId: number };
    } | null;
    ...
    const station = await surveyService.updateSurveyStation(
      id,
      result.data as surveyService.SurveyStationUpdateData,
    );
...
export const deleteSurveyStation = async (req: Request, res: Response) => {
  try {
    if (!ensureCanWriteSurvey(req, res)) {
      return;
    }
...
export const recalculateSurveyStations = async (req: Request, res: Response) => {
  try {
    if (!ensureCanWriteSurvey(req, res)) {
      return;
    }
    ...
    const verticalSectionAzimuth = parseOptionalNumber(
      req.body?.verticalSectionAzimuth,
      "verticalSectionAzimuth",
    );
    ...
    const data = await surveyService.recalculateSurveyStations(
      sessionId,
...
export const importSurveyFromMwdData = async (req: Request, res: Response) => {
  try {
    if (!ensureCanWriteSurvey(req, res)) {
      return;
    }
...
export const importWellPlanCsv = async (req: Request, res: Response) => {
  try {
    if (!ensureCanWriteSurvey(req, res)) {
      return;
    }
    ...
    const verticalSectionAzimuth = parseOptionalNumber(
      body.verticalSectionAzimuth ?? req.query.verticalSectionAzimuth,
      "verticalSectionAzimuth",
    );
