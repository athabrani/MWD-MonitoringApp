...
const parseOptionalBoolean = (value) => {
    ...
    return { provided: true, error: "isDefault must be a boolean" };
};
const isRecord = (value) => {
    return typeof value === "object" && value !== null && !Array.isArray(value);
};
const toRecord = (value) => isRecord(value) ? value : {};
const parseTemplateBody = (body, mode) => {
    const data = {};
...
const handlePlotTemplateError = (error, res) => {
...
export const createPlotTemplate = async (req, res) => {
    try {
        const result = parseTemplateBody(req.body ?? {}, "create");
        if ("error" in result) {
            return res.status(400).json({ message: result.error });
        }
        const template = await plotTemplateService.createPlotTemplate(result.data);
        ...
        await createAuditLog({
            ...
            metadata: {
                plotTemplateId: templateRecord.id !== undefined ? String(templateRecord.id) : null,
                name: templateRecord.name !== undefined ? String(templateRecord.name) : null,
...
export const getAllPlotTemplates = async (_req, res) => {
    try {
...
export const getDefaultPlotTemplate = async (_req, res) => {
    try {
...
export const getPlotTemplateById = async (req, res) => {
    try {
...
export const updatePlotTemplate = async (req, res) => {
    try {
        const id = parsePositiveInt(req.params.id);
        if (id === null) {
            return res.status(400).json({ message: "Invalid plot template id" });
        ...
        const template = await plotTemplateService.updatePlotTemplate(id, result.data);
        const authUser = req.user;
        const templateRecord = toRecord(template);
        await createAuditLog({
            userId: authUser?.userId ?? null,
            ...
            metadata: {
                plotTemplateId: templateRecord.id !== undefined ? String(templateRecord.id) : String(id),
                name: templateRecord.name !== undefined ? String(templateRecord.name) : null,
...
export const deletePlotTemplate = async (req, res) => {
    try {
        const id = parsePositiveInt(req.params.id);
        if (id === null) {
            return res.status(400).json({ message: "Invalid plot template id" });
...
//# sourceMappingURL=plot-template.controller.js.map