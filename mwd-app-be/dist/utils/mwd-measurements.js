...
export const normalizeWitsId = (value) => {
...
export const formatWitsId = (witsId) => {
    return witsId.length === 4
        ? `${witsId.slice(0, 2)} ${witsId.slice(2)}`
        : witsId;
};
const unwrapMeasurementValue = (value) => {
    if (isRecord(value)) {
        if ('value' in value) {
            return value.value;
        }
...
const isNullMeasurementSentinel = (value) => {
    return value === -9999;
};
...
const collectWitsValuesFromRecord = (values, record) => {
...
const collectWitsValuesFromArray = (values, items) => {
    for (const item of items) {
        if (!isRecord(item)) {
            continue;
        }
        const normalizedWitsId = normalizeWitsId(item.witsId ?? item.id ?? item.tag ?? item.word ?? item.code);
        if (!normalizedWitsId) {
            continue;
        }
        values.set(normalizedWitsId, unwrapMeasurementValue(item.value ?? item.reading ?? item.measurement ?? item.data));
    }
};
const parseRawWitsLine = (line) => {
    const trimmed = line.trim();
    if (!trimmed || trimmed === '&&' || trimmed === '!!') {
        return null;
    }
    const separatedMatch = trimmed.match(/^(\d{4})\s*[:=,\t ]\s*([-+]?(?:\d+(?:\.\d+)?|\.\d+)(?:e[-+]?\d+)?)$/i);
    if (separatedMatch) {
    ...
    const compactMatch = trimmed.match(/^(\d{4})([-+]?(?:\d+(?:\.\d+)?|\.\d+)(?:e[-+]?\d+)?)$/i);
    if (compactMatch) {
    ...
    const embeddedMatch = trimmed.match(/(?:^|[^\d])(\d{4})\s*[:=,\t ]\s*([-+]?(?:\d+(?:\.\d+)?|\.\d+)(?:e[-+]?\d+)?)/i);
    if (!embeddedMatch) {
        return null;
    }
...
const collectWitsValuesFromRawText = (values, text) => {
...
export const collectWitsValues = (source) => {
...
export const parseMeasurementFields = (source) => {
    const parsedFields = Object.fromEntries(MWD_MEASUREMENT_FIELDS.map((fieldName) => [
        fieldName,
        parseOptionalDecimal(source[fieldName]),
    ]));
    const witsValues = collectWitsValues(source);
    const appliedWitsFields = new Set();
    const sortedWitsDefinitions = [
        ...WITS_MEASUREMENT_DEFINITIONS,
    ];
    sortedWitsDefinitions.sort((left, right) => (left.priority ?? 0) - (right.priority ?? 0));
    for (const definition of sortedWitsDefinitions) {
        ...
        parsedFields[definition.field] = parseOptionalDecimal(witsValues.get(definition.witsId));
    ...
    return { parsedFields: parsedFields };
};
export const applyMeasurementFields = (target, parsedFields) => {
...
//# sourceMappingURL=mwd-measurements.js.map