...
const getPageHeight = (doc: PDFKit.PDFDocument) => {
  return doc.page.height - doc.page.margins.top - doc.page.margins.bottom
}

const getPageWidth = (doc: PDFKit.PDFDocument) => {
...
export const buildSurveyPdfExport = async (
  rows: SurveyExportRow[],
  fileName: string,
  sessionInfo?: { wellName?: string; rigName?: string },
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      if (sessionInfo?.wellName || sessionInfo?.rigName) {
        let infoText = ''
        if (sessionInfo.wellName) infoText += `Well: ${sessionInfo.wellName}`
