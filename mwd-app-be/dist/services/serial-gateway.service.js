...
import { SerialWitsStreamParser, } from "../utils/serial-wits-parser.js";
...
const readSerialPortText = (portInfo, key) => {
...
const isBluetoothSerialPort = (portInfo) => {
    const text = [
...
const getSignalQuality = (rssi, snr) => {
...
const updateSignalStatus = (metadata, rawLine) => {
    const rssi = parseSignalNumber(metadata.RSSI) ??
        parseSignalNumber(rawLine.match(/RSSI\s*=\s*([-+]?\d+(?:\.\d+)?)/i)?.[1]);
    const snr = parseSignalNumber(metadata.SNR) ??
    ...
    runtimeStatus.signal.quality = getSignalQuality(runtimeStatus.signal.rssi, runtimeStatus.signal.snr);
...
const normalizeChunk = async (data) => {
...
const parseWitsPairs = (text) => {
    const wits = {};
    const normalizedText = text.replace(/\r/g, "\n");
    const separatedLinePattern = new RegExp(`^(\\d{4})\\s*[:,=\\t ]\\s*(${parseNumberString})$`, "i");
    const compactLinePattern = new RegExp(`^(\\d{4})(${parseNumberString})$`, "i");
    const globalPairPattern = new RegExp(`(?:^|[^\\d])(\\d{4})\\s*[:,=\\t ]\\s*(${parseNumberString})`, "gi");
...
const parseMwdKeyValuePayload = (text) => {
    const payload = {};
    const fieldAliases = {
        depth: "depthMd",
        md: "depthMd",
...
const recordConnectionStatus = async (source, status, description) => {
    try {
        ...
        const message = error instanceof Error ? error.message : "Unknown connection log error";
        console.warn(`[Serial GW] Failed to record connection status: ${message}`);
    }
};
const getLatestDepthFromWits = (wits) => {
    return wits["0108"] ?? wits["0110"] ?? null;
};
let SerialPortClassCache = null;
let port = null;
...
const resolveConfiguredSerialPath = async (configuredPath) => {
    ...
    const selectedPath = isRecord(selectedPort) && typeof selectedPort.path === "string"
...
const resetRuntimeStatusForConnect = (options) => {
...
const buildRequiredOptions = (options) => ({
...
const createSerialLineIngestor = (options) => {
    const logIgnoredLine = (message) => {
        const now = Date.now();
    ...
    const ingestSerialLine = async (rawLine) => {
        ...
        if (gatewayPayload.depthMd === undefined &&
            latestDepthMd !== null &&
            depthFromWits === null) {
        ...
        try {
            ...
            if (result.selected) {
                ...
                runtimeStatus.lastError = null;
                console.log(`[Serial GW] Selected ${result.createdItems.length} MWD row(s) from ${options.path}: ${payloadText}`);
            }
            else {
                runtimeStatus.ignoredCount += 1;
    ...
    const ingestSerialWitsBlock = async (block) => {
        runtimeStatus.lastReceivedAt = new Date().toISOString();
        ...
        try {
            ...
            if (result.selected) {
                ...
                runtimeStatus.lastIngestedAt = new Date().toISOString();
                runtimeStatus.lastError = null;
                console.log(`[Serial GW] Selected ${result.createdItems.length} MWD row(s) from ${options.path}: WITS ${Object.keys(block.values).join(", ")}`);
            }
            else {
                runtimeStatus.ignoredCount += 1;
                console.log(`[Serial GW] Candidate skipped: ${result.reason}.`);
...
const openSerialConnection = async () => {
    ...
    port.on("open", () => {
        ...
        void recordConnectionStatus(options.source, "connected", `Serial connected to ${options.path} @ ${options.baudRate}`);
    });
    ...
    port.on("close", () => {
        ...
        runtimeStatus.connected = false;
        void recordConnectionStatus(options.source, "offline", `Serial closed ${options.path}`);
        port = null;
        scheduleReconnect();
    });
    port.open((error) => {
...
export const connectSerialGateway = async (options) => {
...
export const disconnectSerialGateway = async () => {
    ...
    if (runtimeStatus.source) {
        await recordConnectionStatus(runtimeStatus.source, "offline", "Serial gateway disconnected manually");
    }
    return getSerialGatewayStatus();
};
export const startSerialGateway = async () => {
    ...
    const defaultSessionId = parsePositiveInt(process.env.SERIAL_GATEWAY_SESSION_ID ?? process.env.ESP_GATEWAY_SESSION_ID);
    const baudRate = parsePositiveInt(process.env.SERIAL_BAUD_RATE) ?? DEFAULT_BAUD_RATE;
    const reconnectMs = parsePositiveNumber(process.env.SERIAL_GATEWAY_RECONNECT_MS, DEFAULT_RECONNECT_MS);
    ...
    if (defaultSessionId === null) {
        console.warn("[Serial GW] SERIAL_GATEWAY_SESSION_ID is not set. Serial payloads must include sessionId.");
    }
...
//# sourceMappingURL=serial-gateway.service.js.map