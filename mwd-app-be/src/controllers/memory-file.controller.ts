...
export const importMemoryFile = async (req: Request, res: Response) => {
  try {
    ...
    const measuredAtField = parseOptionalString(
      input.measuredAtField ?? input.timeField ?? input.timestampField,
    );
    const hasHeader =
      input.hasHeader === undefined
        ? undefined
        : parseOptionalBoolean(input.hasHeader, true);
    const columns = parseOptionalStringArray(input.columns);
    const fieldMappings = parseJsonLike(
      input.fieldMappings ?? input.mappings ?? input.mapping,
    );
    ...
    const result = await memoryFileService.importMemoryFile({
      ...
      ...(Array.isArray(rows) ? { rows: rows.filter((row): row is Record<string, unknown> => typeof row === "object" && row !== null && !Array.isArray(row)) } : {}),
...
export const getMemoryFiles = async (req: Request, res: Response) => {
  try {
    const authUser = getAuthUser(req);
...
export const getMemoryFileById = async (req: Request, res: Response) => {
  try {
...
export const getMemoryDataPoints = async (req: Request, res: Response) => {
  try {
    const id = parsePositiveInt(req.params.id);
    const limitParam = req.query.limit;
    ...
    const limit =
      typeof limitParam === "string" ? parsePositiveInt(limitParam) : undefined;
    const skip =
      typeof skipParam === "string" ? parseNonNegativeNumber(skipParam) : undefined;
...
export const deleteMemoryFile = async (req: Request, res: Response) => {
  try {
    const authUser = ensureCanModify(req, res);
    const id = parsePositiveInt(req.params.id);
...
export const correlateMemoryFile = async (req: Request, res: Response) => {
  try {
    const authUser = ensureCanModify(req, res);
    const id = parsePositiveInt(req.params.id);
    ...
    const mode =
      body.mode === undefined || body.mode === ""
        ? undefined
        : typeof body.mode === "string" && ["depth", "timestamp"].includes(body.mode)
          ? (body.mode as "depth" | "timestamp")
          : null;
    const depthOffset = parseFiniteNumber(body.depthOffset ?? body.depthShift);
    const measuredAtOffsetMs = parseFiniteNumber(
      body.measuredAtOffsetMs ?? body.timeOffsetMs,
    );
    const maxDepthDifference = parseNonNegativeNumber(
      body.maxDepthDifference ?? body.depthTolerance,
    );
    const maxTimeDifferenceMs = parseNonNegativeNumber(
      body.maxTimeDifferenceMs ?? body.timeToleranceMs,
    );
    ...
    const fieldMappings = parseJsonLike(
      body.fieldMappings ?? body.mappings ?? body.mapping,
    );
...
export const getMemoryCorrelations = async (req: Request, res: Response) => {
  try {
    const authUser = getAuthUser(req);
    const sessionIdParam = req.query.sessionId;
    ...
    const sessionId =
      typeof sessionIdParam === "string" ? parsePositiveInt(sessionIdParam) : undefined;
    const memoryFileId =
      typeof memoryFileIdParam === "string"
