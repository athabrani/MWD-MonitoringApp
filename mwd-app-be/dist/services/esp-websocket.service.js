...
import { broadcastESPGatewayStatus, } from './websocket.service.js';
...
const updateSignalStatus = (metadata, message, rawMessage) => {
    const rssi = parseSignalNumber(message.rssi) ??
        parseSignalNumber(metadata.RSSI) ??
        parseSignalNumber(rawMessage.match(/RSSI\s*=\s*([-+]?\d+(?:\.\d+)?)/i)?.[1]);
    const snr = parseSignalNumber(message.snr) ??
...
const parseWitsPairs = (value) => {
    ...
    for (const line of lines) {
        ...
        const match = line.match(/^(\d{4})\s*[:=]\s*([-+]?\d+(?:\.\d+)?)/) ??
...
const messageDataToString = async (data) => {
...
const recordConnectionStatus = async (source, status, description, responseMs) => {
    try {
...
const toGatewayPayload = (message, defaultSessionId) => {
    ...
    const parsedWitsBlock = !parsedPayload && payload.includes('&&') && payload.includes('!!')
        ? parseSerialWitsBlock(payload)
        : null;
    const gatewayPayload = parsedPayload
        ? { ...parsedPayload }
        : parsedWitsBlock
            ? {
...
export const startEspWebSocketGateway = () => {
    ...
    const reconnectMs = parsePositiveNumber(process.env.ESP_WS_RECONNECT_MS, DEFAULT_RECONNECT_MS);
    const ingestTypes = parseCsvSet(process.env.ESP_WS_INGEST_TYPES, DEFAULT_INGEST_TYPES);
    ...
    if (defaultSessionId === null) {
        console.warn('[ESP WS] ESP_GATEWAY_SESSION_ID is not set. ESP payloads must include sessionId.');
    }
    ...
    const handleMessage = async (rawData) => {
        ...
        const packet = typeof message.data === 'string' ? unwrapLoRaPacket(message.data) : null;
        updateSignalStatus(packet?.metadata ?? {}, message, rawMessage);
        const sessionIdForLog = isRecord(message.data) && message.data.sessionId !== undefined
            ? parsePositiveInt(message.data.sessionId)
            : defaultSessionId;
        const rawMessageRssi = rawMessage.match(/RSSI\s*=\s*([-+]?\d+(?:\.\d+)?)/i)?.[1] ?? null;
        const rawMessageSnr = rawMessage.match(/SNR\s*=\s*([-+]?\d+(?:\.\d+)?)/i)?.[1] ?? null;
        const packetRssi = packet?.metadata.RSSI ?? null;
        const packetSnr = packet?.metadata.SNR ?? null;
        const messageRssi = typeof message.rssi === 'string' || typeof message.rssi === 'number'
            ? message.rssi
            : null;
        const messageSnr = typeof message.snr === 'string' || typeof message.snr === 'number'
            ? message.snr
            : null;
        ...
        try {
            ...
            if (result.selected) {
                ...
                runtimeStatus.lastError = null;
                console.log(`[ESP WS] Selected ${result.createdItems.length} MWD row(s) from ${messageType}.`);
            }
            else {
                runtimeStatus.ignoredCount += 1;
                console.log(`[ESP WS] Candidate skipped: ${result.reason}.`);
            }
    ...
    const connect = () => {
        ...
        socket.addEventListener('open', () => {
            ...
            void recordConnectionStatus(source, 'connected', `Connected to ${url}`, responseMs);
        ...
        socket.addEventListener('error', () => {
            ...
            void recordConnectionStatus(source, 'degraded', `WebSocket error from ${url}`);
            // Broadcast updated gateway status to frontend
            broadcastESPGatewayStatus(getEspWebSocketGatewayStatus());
        });
        socket.addEventListener('close', (event) => {
            ...
            void recordConnectionStatus(source, 'offline', `WebSocket closed (${event.code}${reason})`);
            socket = null;
            // Broadcast updated gateway status to frontend
            broadcastESPGatewayStatus(getEspWebSocketGatewayStatus());
            scheduleReconnect();
...
//# sourceMappingURL=esp-websocket.service.js.map