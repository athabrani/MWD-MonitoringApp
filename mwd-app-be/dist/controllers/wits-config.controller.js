...
import { MWD_MEASUREMENT_FIELDS, normalizeWitsId, } from "../utils/mwd-measurements.js";
...
const parseOptionalText = (value, fieldName) => {
    ...
    return { provided: true, value: value.trim() || null };
};
...
const parseWitsIdInput = (value, fieldName, required) => {
...
const getBodyValue = (body, fieldNames) => {
...
const buildWitsConfigInput = (body, mode) => {
    const data = {};
    const witsId = parseWitsIdInput(getBodyValue(body, ["witsId", "witsID", "wits_id"]), "witsId", mode === "create");
    if ("error" in witsId) {
    ...
    for (const field of textFields) {
        const parsed = parseOptionalText(getBodyValue(body, field.aliases), field.target);
        if ("error" in parsed) {
    ...
    if (typeof data.mappedField === "string" &&
        !MWD_MEASUREMENT_FIELDS.includes(data.mappedField)) {
        return {
            error: `mappedField must be one of: ${MWD_MEASUREMENT_FIELDS.join(", ")}`,
        };
    }
    const customDepthWitsId = parseWitsIdInput(body.customDepthWitsId, "customDepthWitsId", false);
    ...
    for (const field of decimalFields) {
        const parsed = parseOptionalDecimal(getBodyValue(body, field.aliases), field.target);
        if ("error" in parsed) {
            return { error: parsed.error };
        }
        if (parsed.provided) {
    ...
    if (body.decimalPlaces !== undefined) {
        const decimalPlaces = typeof body.decimalPlaces === "number"
            ? body.decimalPlaces
    ...
    for (const field of booleanFields) {
        const parsed = parseBoolean(getBodyValue(body, field.aliases), field.target);
        if ("error" in parsed) {
            return { error: parsed.error };
        }
        if (parsed.provided) {
...
const handleWitsConfigError = (error, res) => {
...
export const createWitsConfig = async (req, res) => {
    try {
        const result = buildWitsConfigInput(req.body ?? {}, "create");
        if ("error" in result) {
            return res.status(400).json({ message: result.error });
        }
        const config = await witsConfigService.createWitsConfig(result.data);
        ...
        await createAuditLog({
            ...
            metadata: {
                witsConfigId: configRecord.id !== undefined ? String(configRecord.id) : null,
                witsId: configRecord.witsId !== undefined
                    ? String(configRecord.witsId)
                    : null,
                mappedField: configRecord.mappedField !== undefined
...
export const getAllWitsConfigs = async (req, res) => {
    try {
...
export const getWitsConfigById = async (req, res) => {
    try {
...
export const updateWitsConfig = async (req, res) => {
    try {
        const id = parsePositiveInt(req.params.id);
        if (id === null) {
            return res.status(400).json({ message: "Invalid WITS config id" });
        ...
        const config = await witsConfigService.updateWitsConfig(id, result.data);
        const authUser = req.user;
        const configRecord = toRecord(config);
        await createAuditLog({
            userId: authUser?.userId ?? null,
            ...
            metadata: {
                witsConfigId: configRecord.id !== undefined ? String(configRecord.id) : String(id),
                witsId: configRecord.witsId !== undefined
                    ? String(configRecord.witsId)
                    : null,
...
export const deleteWitsConfig = async (req, res) => {
    try {
        const id = parsePositiveInt(req.params.id);
        if (id === null) {
            return res.status(400).json({ message: "Invalid WITS config id" });
        ...
        await createAuditLog({
            ...
            metadata: {
                witsConfigId: deletedConfigRecord.id !== undefined
                    ? String(deletedConfigRecord.id)
                    : String(id),
                witsId: deletedConfigRecord.witsId !== undefined
...
//# sourceMappingURL=wits-config.controller.js.map