...
const escapeLasText = (value) => {
    if (value === null || value === undefined || value === "") {
        return "";
    }
    return String(value).replace(/[\r\n]+/g, " ").trim();
};
const sanitizeMnemonic = (value, fallback) => {
...
const makeUniqueMnemonic = (mnemonic, usedMnemonics) => {
...
const getDepthKey = (depth, precision = 4) => depth.toFixed(precision);
const getOrCreateRow = (rows, depth, precision = 4) => {
...
const addCurve = (curves, curve) => {
    if (!curves.has(curve.key)) {
        curves.set(curve.key, curve);
    }
};
const buildMwdWhere = (input) => {
    const where = {
...
const buildDepthWhere = (input, fieldName, includeNotNull = true) => {
    if (input.depthMin === undefined && input.depthMax === undefined) {
        return includeNotNull ? { [fieldName]: { not: null } } : {};
    }
    const depthFilter = includeNotNull ? { not: null } : {};
    if (input.depthMin !== undefined) {
...
const getMwdRows = async (input) => {
    const select = Object.fromEntries(MWD_MEASUREMENT_FIELDS.map((fieldName) => [fieldName, true]));
...
const getSurveyRows = async (input) => {
...
const buildLasCurveSection = (curves) => {
    return curves.map((curve) => `${curve.mnemonic}.${curve.unit} : ${escapeLasText(curve.description)}`);
};
const buildLasAsciiSection = (rows, curves, nullValue, input) => {
    const depthPrecision = input.depthPrecision;
    return rows.map((row) => [
        formatNumberWithPrecision(getAsciiDepth(row, input), depthPrecision),
...
const formatAsciiValue = (value, nullValue) => {
...
const assignUniqueMnemonics = (curves) => {
    const usedMnemonics = new Set(["DEPT"]);
    return curves.map((curve) => ({
        ...curve,
        mnemonic: makeUniqueMnemonic(sanitizeMnemonic(curve.mnemonic, "CURVE"), usedMnemonics),
        unit: escapeLasText(curve.unit).toUpperCase(),
    }));
};
const getAsciiDepth = (row, input) => {
...
const applyColumnSelection = (curves, columns) => {
    if (!columns || columns.length === 0) {
        return curves;
    }
    const curvesByKey = new Map(curves.map((curve) => [curve.key, curve]));
    const curvesByMnemonic = new Map(curves.map((curve) => [curve.mnemonic.toUpperCase(), curve]));
    ...
    for (const column of columns) {
        ...
        const curve = curvesByKey.get(normalizedKey) ??
...
const getCurveValueAtDepth = (sourceRows, curveKey, depth, maxGap) => {
    const exactRow = sourceRows.find((row) => Math.abs(row.depth - depth) < 1e-9 && row.values[curveKey] !== undefined);
    if (exactRow) {
...
const buildSteppedRows = (sourceRows, curves, input) => {
    ...
    const rows = [];
    for (let depth = startDepth; depth <= stopDepth + input.stepDepth / 1000; depth += input.stepDepth) {
        const row = {
            depth,
            values: {},
        };
        for (const curve of curves) {
            const shouldInterpolate = !curve.key.startsWith("survey:") || (input.interpolateSurvey ?? false);
...
const buildAdditionalWellInfoLines = (wellInfo) => {
...
const buildSurveyOtherLines = (surveyRows) => {
...
export const buildLasExport = async (input) => {
    ...
    if (input.includeWits ?? true) {
        ...
        for (const witsRow of witsRows) {
            ...
            const witsConfig = typeof witsRow.witsConfig === "object" && witsRow.witsConfig !== null
                ? witsRow.witsConfig
                : {};
            const witsId = String(witsConfig.witsId ?? witsRow.witsId ?? "");
            const curveKey = `wits:${witsId}`;
            const mnemonic = sanitizeMnemonic(String(witsConfig.lasTag ?? `W${witsId}`), `W${witsId}`);
            addCurve(curves, {
                key: curveKey,
                mnemonic,
                unit: String(witsConfig.units ?? ""),
                description: String(witsConfig.lasDescription ?? witsConfig.name ?? `WITS ${witsId}`),
            });
    ...
    const rawRows = [...rows.values()]
        .filter((row) => input.stopAtLastSurveyDepth && lastSurveyDepth !== undefined
    ...
    const stopDepth = sortedRows[sortedRows.length - 1]
        ? getAsciiDepth(sortedRows[sortedRows.length - 1], input)
        : startDepth;
...
//# sourceMappingURL=las-export.service.js.map