...
const getEditInput = (req: Request): Record<string, unknown> => {
  return (req.method === "GET" ? req.query : req.body ?? {}) as Record<string, unknown>;
};

const parseEditBaseInput = async (req: Request, res: Response) => {
...
const resolveDepthOffset = (body: Record<string, unknown>, depthMin: number) => {
  ...
  const targetStartDepth = parseNonNegativeNumber(
    body.targetStartDepth ?? body.targetDepthStart,
  );
...
export const hideDepthRange = async (req: Request, res: Response) => {
  try {
...
export const unhideDepthRange = async (req: Request, res: Response) => {
  try {
    const base = await parseEditBaseInput(req, res);
...
export const deleteDepthRange = async (req: Request, res: Response) => {
  try {
    const base = await parseEditBaseInput(req, res);
...
export const moveDepthRange = async (req: Request, res: Response) => {
  try {
    const base = await parseEditBaseInput(req, res);
    const input = getEditInput(req);
...
export const previewMoveDepthRange = async (req: Request, res: Response) => {
  try {
    const base = await parseEditBaseInput(req, res);
    const input = getEditInput(req);
...
export const copyDepthRange = async (req: Request, res: Response) => {
  try {
    const base = await parseEditBaseInput(req, res);
    const input = getEditInput(req);
    ...
    const depthOffset = resolveDepthOffset(input, base.depthMin);
    const measuredAtOffsetMs =
      input.measuredAtOffsetMs === undefined
        ? undefined
...
export const previewCopyDepthRange = async (req: Request, res: Response) => {
  try {
    const base = await parseEditBaseInput(req, res);
    const input = getEditInput(req);
...
export const rescaleDepthRange = async (req: Request, res: Response) => {
  try {
    const base = await parseEditBaseInput(req, res);
    const input = getEditInput(req);
    ...
    if (!MWD_MEASUREMENT_FIELDS.includes(field as MeasurementField)) {
      return res.status(400).json({
    ...
    const result = await mwdDataEditService.rescaleDepthRange({
      ...base,
      field: field as MeasurementField,
      scaleFactor,
      biasOffset,
    });
...
export const previewRescaleDepthRange = async (req: Request, res: Response) => {
  try {
    const base = await parseEditBaseInput(req, res);
    const input = getEditInput(req);
...
export const getEditOperations = async (req: Request, res: Response) => {
  try {
