...
import { buildExportFileName, buildSurveyExportFileName, buildWitsExportFileName, serializeHistoricalDataAsCsv, serializeHistoricalDataAsJson, serializeSurveyStationsAsCsv, serializeWitsValuesAsCsv, } from '../services/export.service.js';
...
const parseOptionalSignedNumber = (value) => {
    if (value === undefined || value === null || value === '') {
        return undefined;
    }
...
export const exportHistoricalData = async (req, res) => {
    try {
        ...
        const historicalData = await historicalDataService.getHistoricalData(historicalQuery);
        ...
        res.setHeader('Content-Type', format === 'json'
...
export const exportHistoricalLast24Hours = async (req, res) => {
    try {
        const authUser = req.user;
        if (!authUser) {
            return res.status(401).json({ message: 'Unauthorized' });
        }
        const parsedSessionId = req.query.sessionId === undefined
            ? undefined
        ...
        let session = null;
        const sessionId = typeof parsedSessionId === 'number' ? parsedSessionId : undefined;
        ...
        const fileName = buildExportFileName(safeSessionCode, format).replace('_historical_', '_historical_last_24_hours_');
        const body = format === 'json'
            ? serializeHistoricalDataAsJson(historicalData.data)
            : serializeHistoricalDataAsCsv(historicalData.data);
        if (sessionId !== undefined) {
...
export const exportWitsData = async (req, res) => {
    try {
        const authUser = req.user;
        if (!authUser) {
            return res.status(401).json({ message: 'Unauthorized' });
        ...
        const sampleMode = req.body?.sampleMode === 'first_per_depth' ? 'first_per_depth' : 'all';
        if (sessionId === null) {
            return res.status(400).json({ message: 'Valid sessionId is required' });
        }
        if (witsId === null) {
        ...
        if ((req.body?.depthMin !== undefined || req.body?.startDepth !== undefined) &&
            depthMin === null) {
            return res
                .status(400)
                .json({ message: 'startDepth/depthMin must be a non-negative number' });
        }
        if ((req.body?.depthMax !== undefined || req.body?.endDepth !== undefined) &&
            depthMax === null) {
            return res
                .status(400)
        ...
        const configName = isRecord(config) && typeof config.name === 'string'
            ? config.name
            : undefined;
...
export const exportLasData = async (req, res) => {
    try {
        const authUser = req.user;
        if (!authUser) {
            return res.status(401).json({ message: 'Unauthorized' });
        ...
        const includeSurvey = parseOptionalBoolean(req.body?.includeSurvey ?? req.body?.includeProjectedSurvey, true);
        ...
        const stopAtLastSurveyDepth = parseOptionalBoolean(req.body?.stopAtLastSurveyDepth, false);
        const dateTimeInFirstColumn = parseOptionalBoolean(req.body?.dateTimeInFirstColumn, false);
        const correctDepthColumnForTvd = parseOptionalBoolean(req.body?.correctDepthColumnForTvd, false);
        const interpolateSurvey = parseOptionalBoolean(req.body?.interpolateSurvey, false);
        const includeSurveysInOtherSection = parseOptionalBoolean(req.body?.includeSurveysInOtherSection, false);
        const columns = req.body?.columns === undefined
            ? undefined
            : Array.isArray(req.body.columns)
                ? req.body.columns
                : 'invalid';
        const wellInfo = req.body?.wellInfo === undefined
            ? undefined
            : Array.isArray(req.body.wellInfo)
                ? req.body.wellInfo
                : 'invalid';
        const depthUnit = typeof req.body?.depthUnit === 'string' && req.body.depthUnit.trim()
            ? req.body.depthUnit.trim()
            : undefined;
        const surveyStationType = typeof req.body?.surveyStationType === 'string' &&
        ...
        if ((req.body?.depthMin !== undefined ||
        ...
        if (columns !== undefined && columns !== 'invalid') {
            lasInput.columns = columns;
        }
        if (wellInfo !== undefined && wellInfo !== 'invalid') {
            lasInput.wellInfo = wellInfo;
        }
...
export const exportSurveyData = async (req, res) => {
    try {
        ...
        const stationType = typeof req.body?.stationType === 'string' && req.body.stationType.trim()
        ...
        const body = serializeSurveyStationsAsCsv(stations);
        await exportRecordService.createExportRecord({
            sessionId,
            exportedById: authUser.userId,
            fileName,
...
export const exportSurveyDataAsExcel = async (req, res) => {
    try {
        const authUser = req.user;
        if (!authUser) {
            return res.status(401).json({ message: 'Unauthorized' });
        ...
        const fileName = buildSurveyExportFileName(session.sessionCode, stationType).replace('.csv', '.xlsx');
        const excelExport = await buildSurveyExcelExport(stations, fileName);
        await exportRecordService.createExportRecord({
            sessionId,
            exportedById: authUser.userId,
        ...
        res.setHeader('Content-Disposition', `attachment; filename="${excelExport.fileName}"`);
...
export const exportSurveyDataAsPdf = async (req, res) => {
    try {
        const authUser = req.user;
        if (!authUser) {
            return res.status(401).json({ message: 'Unauthorized' });
        ...
        const fileName = buildSurveyExportFileName(session.sessionCode, stationType).replace('.csv', '.pdf');
        const sessionInfo = {};
        if (session.wellName) {
        ...
        const pdfExport = await buildSurveyPdfExport(stations, fileName, sessionInfo);
        await exportRecordService.createExportRecord({
            sessionId,
            exportedById: authUser.userId,
...
export const exportPdfPlot = async (req, res) => {
    try {
        const authUser = req.user;
        if (!authUser) {
            return res.status(401).json({ message: 'Unauthorized' });
...
export const getExportRecords = async (_req, res) => {
    try {
