...
const determineStatus = (bitDepth, holeDepth, rop, fallback) => {
...
const toDateWhere = (measuredFrom, measuredTo) => {
    const measuredAt = {};
    if (measuredFrom) {
...
export const buildDepthTrackingInputFromMwdSource = (input) => {
    ...
    const bitDepth = toFiniteNumber(witsValues.get("0108")) ??
        toFiniteNumber(input.source.bitDepth) ??
        toFiniteNumber(input.source.depthMd);
    const holeDepth = toFiniteNumber(witsValues.get("0110")) ??
        toFiniteNumber(input.source.holeDepth) ??
        toFiniteNumber(input.source.depthMd);
    const blockDepth = toFiniteNumber(witsValues.get("0112")) ??
        toFiniteNumber(input.source.blockDepth) ??
        toFiniteNumber(input.source.hookPosition);
    const rop = toFiniteNumber(witsValues.get("0113")) ??
...
export const updateDepthTrackingState = async (input, client = prisma) => {
...
export const getDepthTrackingState = async (sessionId) => {
    return await db().depthTrackingState.findUnique({
        where: { sessionId },
        select: depthTrackingStateSelect,
    });
};
export const getDepthTrackingSamples = async (filters) => {
...
export const recalculateDepthTrackingFromMwdData = async (sessionId) => {
...
//# sourceMappingURL=depth-tracking.service.js.map