...
type MoveDepthInput = EditOperationInput &
  DepthRangeOptions & {
    depthOffset: number;
  };

type CopyDepthInput = EditOperationInput &
  DepthRangeOptions & {
    depthOffset: number;
    measuredAtOffsetMs?: number;
  };

type RescaleInput = EditOperationInput &
  DepthRangeOptions & {
...
const normalizeDepthRange = (depthMin: number, depthMax: number) => {
  return {
...
const buildRangeWhere = (
  input: EditOperationInput & DepthRangeOptions,
) => {
...
const createEditOperation = async (
...
const getDepthRangePreviewRows = async (
  input: EditOperationInput & DepthRangeOptions,
) => {
...
export const previewMoveDepthRange = async (input: MoveDepthInput) => {
...
export const previewCopyDepthRange = async (input: CopyDepthInput) => {
  const where = buildRangeWhere(input);
  const [affectedCount, rows] = await Promise.all([
    db().mWDData.count({ where }),
    getDepthRangePreviewRows(input),
...
export const previewRescaleDepthRange = async (input: RescaleInput) => {
  const where = {
...
export const setHiddenByDepthRange = async (
  input: EditOperationInput & DepthRangeOptions & { hidden: boolean },
) => {
  const result = await db().$transaction(async (tx) => {
    ...
    await createEditOperation(
      tx,
      input,
      input.hidden ? "hide_depth_range" : "unhide_depth_range",
      update.count,
      { hidden: input.hidden },
    );
...
export const deleteDepthRange = async (
  input: EditOperationInput & DepthRangeOptions,
) => {
  const result = await db().$transaction(async (tx) => {
...
export const moveDepthRange = async (input: MoveDepthInput) => {
  const result = await db().$transaction(async (tx) => {
...
export const copyDepthRange = async (input: CopyDepthInput) => {
  const result = await db().$transaction(async (tx) => {
    ...
    const rowsToCreate = rows
      .map((row) => {
        ...
        const data: Record<string, unknown> = {
          ...
          measuredAt: new Date(
            row.measuredAt.getTime() + (input.measuredAtOffsetMs ?? 0),
          ),
...
export const rescaleDepthRange = async (input: RescaleInput) => {
  const result = await db().$transaction(async (tx) => {
    const rows = await db(tx).mWDData.findMany({
      where: {
        ...buildRangeWhere(input),
...
export const getEditOperations = async (
