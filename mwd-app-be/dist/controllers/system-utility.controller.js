...
const parseDepthValue = (value, fallback) => {
...
const getClearInput = (body) => {
    const sessionId = parsePositiveInt(body.sessionId);
    const startDepth = parseDepthValue(body.startDepth ?? body.depthMin ?? body.depthStart, 0);
    const endDepth = parseDepthValue(body.endDepth ?? body.depthMax ?? body.depthEnd, 99999);
    ...
    if (startDepth === null || endDepth === null) {
        return { error: "startDepth and endDepth must be valid numbers" };
    }
    if (startDepth > endDepth) {
        return { error: "startDepth must be less than or equal to endDepth" };
    }
    return {
        sessionId,
        depthRange: {
...
export const getClearDataTargets = (_req, res) => {
    res.json({
        data: systemUtilityService.getValidTargets(),
    });
};
export const getConfigurationBackupTargets = (_req, res) => {
    res.json({
        data: systemUtilityService.getValidConfigurationTargets(),
    });
};
export const backupSessionData = async (req, res) => {
    try {
        const input = getClearInput(req.body ?? {});
        if ("error" in input) {
            return res.status(400).json({ message: input.error });
        }
        const result = await systemUtilityService.createSessionBackup(input.sessionId, input.depthRange, input.targets);
...
export const previewClearSessionData = async (req, res) => {
    try {
        const input = getClearInput(req.body ?? {});
        if ("error" in input) {
            return res.status(400).json({ message: input.error });
        }
        const result = await systemUtilityService.previewClearSessionData(input.sessionId, input.depthRange, input.targets);
        await createAuditLog({
            userId: getUserId(req),
...
export const clearSessionData = async (req, res) => {
    try {
        const input = getClearInput(req.body ?? {});
        if ("error" in input) {
            return res.status(400).json({ message: input.error });
        ...
        const result = await systemUtilityService.clearSessionData(input.sessionId, input.depthRange, input.targets);
        await createAuditLog({
            userId: getUserId(req),
...
export const restoreSessionData = async (req, res) => {
    try {
        ...
        const targets = systemUtilityService.normalizeTargets(req.body?.targets ?? (typeof backup === "object" && backup !== null
            ? backup.targets
            : undefined));
        const replaceExisting = parseBoolean(req.body?.replaceExisting);
        const result = await systemUtilityService.restoreSessionData(sessionId, backup, targets, replaceExisting);
        await createAuditLog({
            userId: getUserId(req),
...
export const backupConfiguration = async (req, res) => {
    try {
        const targets = systemUtilityService.normalizeConfigurationTargets(req.body?.targets);
...
export const restoreConfiguration = async (req, res) => {
    try {
        ...
        const targets = systemUtilityService.normalizeConfigurationTargets(req.body?.targets ??
            (typeof backup === "object" && backup !== null
                ? backup.targets
                : undefined));
        const result = await systemUtilityService.restoreConfigurationBackup(backup, targets);
        await createAuditLog({
            userId: getUserId(req),
...
//# sourceMappingURL=system-utility.controller.js.map