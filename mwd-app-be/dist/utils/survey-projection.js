...
const normalizeDeltaAngle = (value) => {
...
const roundSurveyValue = (value, decimalPlaces = 4) => {
...
const calculateClosureAzimuth = (northing, easting) => {
    if (northing === 0 && easting === 0) {
        return 0;
    }
    return normalizeAzimuth(radiansToDegrees(Math.atan2(easting, northing)));
};
export const projectSurveyStations = (stations, options = {}) => {
    const sortedStations = [...stations].sort((left, right) => left.measuredDepth - right.measuredDepth);
    if (sortedStations.length === 0) {
        return [];
    }
    const verticalSectionAzimuth = normalizeAzimuth(options.verticalSectionAzimuth ?? sortedStations[0]?.azimuth ?? 0);
    ...
    for (const [index, station] of sortedStations.entries()) {
        if (index === 0) {
            ...
            projectedStations.push({
                ...
                verticalSection: roundSurveyValue(northing * Math.cos(verticalSectionAzimuthRadians) +
        ...
        const doglegRadians = Math.acos(clamp(Math.cos(previousInclination) * Math.cos(currentInclination) +
            Math.sin(previousInclination) *
                Math.sin(currentInclination) *
                Math.cos(currentAzimuth - previousAzimuth), -1, 1));
        const ratioFactor = Math.abs(doglegRadians) < 1e-12
            ? 1
            : (2 / doglegRadians) * Math.tan(doglegRadians / 2);
        const deltaTvd = (courseLength / 2) *
            (Math.cos(previousInclination) + Math.cos(currentInclination)) *
            ratioFactor;
        const deltaNorthing = (courseLength / 2) *
            (Math.sin(previousInclination) * Math.cos(previousAzimuth) +
                Math.sin(currentInclination) * Math.cos(currentAzimuth)) *
            ratioFactor;
        const deltaEasting = (courseLength / 2) *
        ...
        const verticalSection = northing * Math.cos(verticalSectionAzimuthRadians) +
            easting * Math.sin(verticalSectionAzimuthRadians);
        const closureDistance = Math.hypot(northing, easting);
        const doglegSeverity = (radiansToDegrees(doglegRadians) * 100) / courseLength;
        const buildRate = ((station.inclination - previousInput.inclination) * 100) / courseLength;
        const turnRate = (normalizeDeltaAngle(station.azimuth - previousInput.azimuth) * 100) /
            courseLength;
        projectedStations.push({
            ...station,
            tvd: roundSurveyValue(tvd),
...
//# sourceMappingURL=survey-projection.js.map