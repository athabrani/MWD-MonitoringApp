...
const buildErrorResponse = (error, res) => {
    ...
    const log = typeof error === "object" && error !== null && "log" in error
        ? error.log
        : undefined;
...
export const getEmailReportLogs = async (req, res) => {
    try {
        const authUser = getAuthUser(req);
        ...
        const filteredLogs = authUser && canViewAllSessions(authUser.roleName)
            ? logs
            : logs.filter((log) => {
...
//# sourceMappingURL=email-report.controller.js.map