...
export const getDepthTrackingState = async (req, res) => {
    try {
...
export const getDepthTrackingSamples = async (req, res) => {
    try {
        const sessionId = parsePositiveInt(req.query.sessionId);
        const measuredFrom = parseOptionalDate(req.query.measuredFrom);
        const measuredTo = parseOptionalDate(req.query.measuredTo);
        const limit = req.query.limit === undefined ? undefined : parsePositiveInt(req.query.limit);
        if (sessionId === null) {
            return res.status(400).json({ message: "sessionId must be a positive integer" });
        }
...
export const updateDepthTrackingState = async (req, res) => {
    try {
...
export const recalculateDepthTracking = async (req, res) => {
    try {
        if (!ensureCanModify(req, res)) {
            return;
        }
        ...
        const result = await depthTrackingService.recalculateDepthTrackingFromMwdData(sessionId);
        res.json({
...
//# sourceMappingURL=depth-tracking.controller.js.map