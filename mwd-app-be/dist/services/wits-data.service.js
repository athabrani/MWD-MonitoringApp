...
import { collectWitsValues, formatWitsId, normalizeWitsId, } from "../utils/mwd-measurements.js";
import { getEnabledWitsConfigsByIds, witsConfigSelect, } from "./wits-config.service.js";
...
const getRawWitsMetadata = (source, witsId, fallbackRawValue) => {
    const rawBlock = typeof source.rawWitsBlock === "string"
        ? source.rawWitsBlock
    ...
    for (const item of serialWitsLines) {
        ...
        return {
            rawText: line.rawValue === undefined || line.rawValue === null
                ? null
    ...
    return {
        rawText: fallbackRawValue === undefined || fallbackRawValue === null
            ? null
...
const getMeasuredAtWhere = (filters) => {
    const measuredAt = {};
...
export const recordConfiguredWitsValues = async (input, db = prisma) => {
    const rawWitsValues = collectWitsValues(input.source);
    const witsIds = Array.from(rawWitsValues.keys());
    const configs = (await getEnabledWitsConfigsByIds(witsIds, db));
    const values = [];
    const alarms = [];
    ...
    for (const config of configs) {
        ...
        const rawMetadata = getRawWitsMetadata(input.source, config.witsId, rawInputValue);
        ...
        const customDepth = customDepthWitsId !== null
        ...
        const alarm = alarmMin !== null && value < alarmMin
...
export const getWitsDataValues = async (filters, db = prisma) => {
    const where = {};
...
export const getWitsDataValuesForExport = async (filters, db = prisma) => {
    const where = {
    ...
    for (const row of rows) {
        const depthKey = row.depthMd === null || row.depthMd === undefined
...
export const getWitsAlarmEvents = async (filters, db = prisma) => {
    const where = {};
    if (filters.sessionId !== undefined) {
        where.sessionId = filters.sessionId;
    }
...
export const acknowledgeWitsAlarm = async (id, acknowledgedById, db = prisma) => {
...
export const resolveWitsAlarm = async (id, db = prisma) => {
    return await client(db).witsAlarmEvent.update({
        where: { id },
        data: {
            resolvedAt: new Date(),
...
//# sourceMappingURL=wits-data.service.js.map