...
const normalizeRawValue = (value: string) => value.trim().replace(/^[:,=\t ]+/, "").trim();
...
export const parseSerialWitsLine = (rawLine: string): ParsedSerialWitsLine => {
...
export const parseSerialWitsBlock = (rawBlock: string): ParsedSerialWitsBlock => {
  ...
  const malformedLines: ParsedSerialWitsLine[] = [];
...
export class SerialWitsStreamParser {
  ...
  push(chunk: string): SerialWitsStreamResult {
    const result: SerialWitsStreamResult = {
      blocks: [],
      standaloneLines: [],
    };
  ...
  private processLine(rawLine: string, result: SerialWitsStreamResult) {
